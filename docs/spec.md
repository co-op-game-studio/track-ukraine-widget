...
- AC-59.2: The CLI SHALL iterate every row in `bills` ordered by `bill_id ASC`, calling the extracted `importBillCore({ congress, type, number, force: true, actorEmail, traceId, d1, kv, congressApiKey })` per row. The CLI SHALL emit one stdout line per bill processed in the format `[backfill] {bill_id} {ok|FAILED} {ms}ms { ...counters }` so the CI log is grep-able.
...
- AC-59.5: After a clean run, the query `SELECT became_law, COUNT(*) FROM bills GROUP BY became_law` SHALL return at least 4 rows with `became_law=1` (HR-815, HR-2471, HR-6968, HR-7108 — the four known-PL bills in the current curated set). An integration test against `importBillCore` SHALL simulate Congress.gov returning `laws=[{number:'117-109'}]` and assert `bills.became_law=1` in the row after import.
...
- AC-59.8: A researcher-visible **Data freshness panel** in the admin SPA (under Settings ▸ Poll Status, consistent with the existing pattern) SHALL display **research-relevant state** about the bill corpus, not operator-relevant CI state. The panel SHALL show: total bill count, count per congress, count per direction, the distribution of `bills.last_freshness_check_at` ("N bills last refreshed within 24h / 7d / 30d / older"), and a "stale bills" list (bills with `last_freshness_check_at` older than 24h, capped at 20). The panel SHALL NOT show operator concepts ("cron tick N", "job stalled at chunk M", "next run at HH:MM"). The standalone CLI's audit_log rows are the contract; the panel reads `audit_log` for the "last refresh attempted: <timestamp>" header but presents that information as a research-facing observation, not as a job-control surface. Long-term home of this panel is open and tracked under the post-v4.1.0 minisite-platform planning.
