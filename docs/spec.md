...
- AC-26.11 (NEW v2.5.1, revised 2026-04-18): The deployed static-asset surface (currently Worker Sites, binding `ASSETS`, backed by `./dist`) SHALL contain, at minimum: `voter-info-widget.iife.js`, `voter-info-widget.iife.js.sri`, `ukraineBills.json`, `ukraineVotes.json`. The deploy workflow SHALL ship all four on every deploy; a post-deploy smoke test SHALL `curl -I` each path on the deployed hostname and assert a 200 response. Rationale: missing datasets were found in prod during the 2026-04-17 audit — the Worker returned 404 for `/ukraineVotes.json` and `/ukraineBills.json` because neither was ever uploaded. Making asset presence + verification part of the deploy contract prevents silent loss of functionality. **Historical note:** the original v2.5.1 wording referenced an R2 bucket; main's migration to Worker Sites (ADR-011) moved static assets to the `./dist` directory bundled with the Worker deploy. The AC's substance — four files, four smoke checks — is unchanged.
