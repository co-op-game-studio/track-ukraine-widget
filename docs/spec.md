...
- AC-49.4: After the bootstrap seed lands in an environment, edits to `src/data/ukraineBills.json` SHALL NOT propagate to that environment. The file SHALL carry a top-of-file comment `// FROZEN — see FR-49. Edits go through the admin front-end (FR-52).` A CI check (`scripts/json-frozen-check.mjs`, deferred under T-128) MAY enforce this; the comment alone is the v2.7.0 v1 enforcement.
...
- AC-51.4: The `comment:v1:{billId}` record SHALL be `{ billId, comments: ResearcherComment[], generatedAt, schemaVersion: 1 }` where each comment is `{ id, bodyMarkdown, scoreAdjustment: number, attachedToRollCallId?: string, authorEmail, createdAt, updatedAt }`. `scoreAdjustment` is a real number; positive values pull the score toward +1, negative values toward −1, in the same units as a per-vote contribution before weighting.
- AC-51.5: The `social-post:v1:{bioguideId}` record SHALL be `{ bioguideId, posts: SocialPost[], generatedAt, schemaVersion: 1 }` where each post is `{ id, platform: "x" | "facebook" | "youtube" | "instagram" | "other", url, postedAt, bodyText, scoreAdjustment, comment?, authorEmail, createdAt }`.
- AC-51.6: The `quote:v1:{bioguideId}` record SHALL be `{ bioguideId, quotes: Quote[], generatedAt, schemaVersion: 1 }` where each quote is `{ id, mediaKind: "video" | "audio" | "text" | "image", sourceUrl, sourceLabel, quotedAt?, bodyText, scoreAdjustment, comment?, authorEmail, createdAt }`.
...
**Solution.** A new Vite entry `src/researcher/main.tsx` produces a small React SPA served by the Worker at `/researcher`. The SPA has tabs for: Bills, Votes, Comments, Social Posts, Quotes, Recent Activity. Each tab is a list-detail layout: list on the left, drawer-style editor on the right. Save → POST to `/api/admin/*` → optimistic update → revalidate from server. No router (single-page tabs); no global state library (component-local `useState` + a small `fetcher` hook is enough at MVP).
...
- AC-53.3: Each comment / post / quote SHALL render a "score adjustment" chip when `scoreAdjustment !== 0`: `+0.05`, `-0.10`, etc., color-coded with the same green/red gradient used for valence (`scoreToCssColor`-style hue). Zero-adjustment items render no chip — the editorial neutrality is the signal.
...
Researcher comments with non-zero `scoreAdjustment` (FR-51 AC-51.4) SHALL be applied **before** shrink, so a researcher can override the prior by attaching evidence-bearing commentary.
...
- AC-58.1: `GET /api/admin/audit?limit=N` (with valid Access header) SHALL return up to N audit rows (max 100, default 50), most-recent first, full row data: `{ id, actor_email, action, table, row_id, row_title, before, after, reason, created_at }`.
- AC-58.2: `GET /api/audit/public?limit=N` (no auth required) SHALL return up to N audit rows (max 50, default 20) in the redacted projection: `{ id, actorLocalPart, action, table, rowTitle, createdAt }`. Email domain SHALL be stripped (`alice@example.com` → `alice`). `before`/`after`/`reason` SHALL NOT be exposed.
