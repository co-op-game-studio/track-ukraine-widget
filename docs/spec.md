...
- AC-27.21 (v2.5.1, REVISED v2.5.2): The Worker SHALL enforce a **per-IP in-Worker rate limit** on `/api/*` requests via the Cloudflare Workers Rate Limiting API binding (`RATE_LIMITER`). The binding SHALL be declared in `wrangler.toml` with a key derived from `cf.connecting_ip` (falling back to the `CF-Connecting-IP` header) for every `/api/*` request, evaluated **after** origin validation (so rejected-origin requests do not consume the budget). On limit exceeded, the Worker SHALL return `429 Too Many Requests` with body `{"error":"rate_limited","retry_after":<seconds>}`, `Retry-After: <seconds>` header, and `Cache-Control: no-store`. Per-env limits:
  ...
  **Per-visit fan-out worst case (v2.5.2 reality check, cold edge cache):**
  - Address flow (3 reps resolved): 1 census + 2 member-list + 3 `/api/members/{id}` = **6 requests**.
  - Exploring one House rep with N curated votes: 1 `/api/members/{id}` + N `/api/congress/v3/house-vote/{c}/{s}/{rc}/members` (one per curated vote). At the current curator count of 18 House Ukraine votes that is **~19 requests**.
  - Exploring one Senate rep with M curated votes: 1 `/api/members/{id}` + M `/api/senate/legislative/LIS/.../xml` (one per curated vote). At the current count of 26 Senate Ukraine votes that is **~27 requests**.
  - Exploring all three reps on a single cold-cache visit: **~70+ requests in the first few seconds** until the edge cache fills.

  The prior 10/60s prod ceiling (v2.5.1) was below a single cold rep click and 429'd legitimate go-live traffic on 2026-04-18. 300/60s gives comfortable headroom for one visitor opening multiple reps without hitting the limit, while still gating abuse.

  **Forward path:** FR-32 AC-32.15 (roll-call rosters in KV) and AC-32.16 (state-members in KV) eliminate the per-visit roll-call fan-out. Once those land and the widget routes through them, the per-visit worst case drops to ~10 requests and AC-27.21 prod/stg SHALL be re-tightened in a follow-up revision. The 300/60s value is an **interim** ceiling until that refactor ships.
...
- AC-28.3 (REVISED v2.5.2): The zone SHALL have a **Rate Limiting Rule** on `/api/*` configured to **600 requests per 60 seconds per client IP in prod**, **600 rpm in stg**, **600 rpm in uat**, **2400 rpm in dev**. Match expression: `(starts_with(http.request.uri.path, "/api/") and http.host eq "<env-hostname>")` — one rule per env hostname to permit per-env thresholds. Action: `Block` with a 10-minute timeout. Response: `429 Too Many Requests` (CF default block page; our Worker's JSON envelope doesn't apply at this layer).

  Rationale (revised v2.5.2): the v2.5.1 values (20/60s in prod) were authored against a fan-out estimate of ~7 upstream calls per visit; v2.5.2 reality (see AC-27.21 fan-out breakdown) is **~70 requests on a cold-cache visit that explores all three reps**. A zone limit lower than the Worker limit makes the Worker limit dead code — the zone always rejects first. The zone rule MUST stay at least **2× AC-27.21** so that the Worker's fine-grained limiter is the binding constraint and the zone rule catches only flagrant abuse (e.g., diverse-IP volumetric attacks where individual IPs stay under the Worker rule).

  **Layered with AC-27.21** (Worker per-IP limit of 300/60s in prod): the zone rule is the blunt volumetric outer gate; the Worker rule is the fine-grained, per-env, quota-aware inner gate. An attacker coming from one IP hits the Worker limit first (which still counts rejected requests against CF's bot signals); an attacker coming from diverse IPs that each stay under the Worker limit hits the zone limit once aggregate volume from that /24 or ASN exceeds 600/min/IP.

  **Forward path:** when FR-32 AC-32.15/16 roll-call + state-member KV families land and the widget fan-out drops to ~10 requests per visit, AC-28.3 SHALL be re-tightened in lockstep with AC-27.21 (targeting 40/60s prod, 2× the Worker limit).
