...
- AC-28.3: The zone SHALL have a **Rate Limiting Rule** on `/api/*` limited to **100 requests per minute per client IP**. Match expression: `(http.host matches "^.*vote\.cogs\.it\.com$" and starts_with(http.request.uri.path, "/api/"))`. Action: `Block` with a 10-minute timeout. Response: `429 Too Many Requests` (CF default response suffices; our Worker's JSON envelope doesn't apply since the rule runs before the Worker). Rationale: legitimate voters hit `/api/*` at most a handful of times per address lookup; the edge cache absorbs the common case; 100/min/IP is comfortably above any legitimate usage and well below what would meaningfully drain our 5000-req/hour Congress.gov budget.
