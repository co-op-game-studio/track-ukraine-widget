...
### FR-59: Standalone Bill Backfill CLI + Full Roll-Call Ingest (NEW v4.1.0)
...
**Solution.** Move backfill out of the runtime stack entirely. The new home is a **standalone tsx CLI** at `scripts/backfill-bills.ts`, run by GitHub Actions on a cron schedule (and on-demand via `workflow_dispatch`). The CLI:
...
- Exits when done. Has no Worker presence. Has no SPA presence. The admin SPA's `useAutoBackfill` hook and the `/api/admin/backfill-bills` route are **deleted**, not deprecated.
...
**Note on the orchestrator.** `importBillFromCongress` currently lives in `proxy/services/import-bill.ts` and pulls `D1` + `KV` bindings from `ProxyEnv`. To consume it from a CLI, the orchestration logic SHALL be extracted into `scripts/lib/import-bill-core.ts` (pure helper, no Worker types), with the existing `proxy/services/import-bill.ts` becoming a thin adapter that wires Worker bindings into the core. The deleted SPA-driven route (`/api/admin/backfill-bills`) was the *only* runtime caller and goes away; `importBillFromCongress` itself stays exported from the adapter for any future per-bill admin actions.
...
- AC-59.1: A standalone CLI program SHALL exist at `scripts/backfill-bills.ts`, invoked via `tsx scripts/backfill-bills.ts --env <dev|uat|stg|prod> [--limit N] [--after <bill_id>] [--dry-run]`. The program SHALL exit with code 0 on success (all bills processed or budget exhausted), code 1 on configuration error (missing API key, unreachable D1), and code 2 on at-least-one-bill-failed (partial success — failures logged to audit_log; cron remains green by treating exit 2 as a warning, not a failure).
- AC-59.2: The CLI SHALL iterate every row in `bills` ordered by `bill_id ASC`, calling the extracted `importBillCore({ congress, type, number, force, actorEmail, traceId })` per row with deps `{ d1, congressClient, auditLog, kvInvalidate, log }`. The `force` flag defaults to `false` so the bill-level freshness gate (per AC-59.10) short-circuits unchanged bills to one API call each; `--force` on the CLI flips it to `true` for a full re-pull. The CLI SHALL emit one stdout line per bill processed in the format `[backfill] {bill_id} {ok|cached|FAILED} {ms}ms { ...counters }` so the CI log is grep-able.
- AC-59.3: Per-bill failures SHALL NOT abort the loop. Each failure SHALL be written to `audit_log` with `action='bill_backfill_error'`, `actor_email='ci@backfill'`, `row_id=<bill_id>`, `reason=<upstream_error_message>`, `trace_id=<run_trace_id>`. The CLI continues to the next bill. The final exit summary SHALL include `{ processed, ok, failed, errors: [...] }`.
...
- AC-59.10: Idempotency + minimum-fetch posture. Once a bill is saved, it SHALL NOT be re-fetched in full unless Congress.gov reports it has changed. The mechanism:
  ...
  - **Default `force=false`:** the CLI runs without `--force` by default. The freshness gates are the cost-control mechanism; `--force` exists only for "I just edited the seed and want to re-pull everything."
  - **Expected steady-state cost:** for the current 63-bill corpus, a no-changes run SHALL produce ≤ 63 API calls (~45s at 5,000/h ceiling). A cold run with no prior data SHALL produce ≤ 500 API calls. An integration test SHALL verify the no-changes case writes zero new `bill_cosponsors`, `votes`, or `bill_actions` rows on the second run.
...
- AC-59.11: A GitHub Actions workflow `.github/workflows/backfill-bills.yml` SHALL drive the CLI: cron schedule (default: every 6h on dev/uat, every 12h on stg/prod), `workflow_dispatch` for manual runs, per-env matrix, secrets-store wiring (`CONGRESS_API_KEY_<ENV>`, `D1_API_TOKEN_<ENV>`). Workflow exits green on CLI exit 0 *or* 2 (partial success — failures are in audit_log, not in CI alerts); exits red on CLI exit 1 (config error).
