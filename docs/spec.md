...
#### FR-32 addendum (v4.1.1/v4.1.2): shared vote resolver, cross-chamber records, durable D1 casts

**Problem.** A member's per-roll-call casts ("did they vote Aye/Nay?") have historically lived ONLY in KV `roll-call-roster:v1:*`, populated by an ops warmer/publish step from upstream. They are not durable (a KV wipe loses them) and the widget resolved a member against only their *current* chamber — so a chamber-switcher (e.g. Adam Schiff, S001150: House 2001–2024, now Senate) showed an empty/insufficient record because the resolver never looked at the chamber they served in. This addendum (a) extracts a shared resolver, (b) resolves across both chambers, and (c) makes casts durable D1-seeded data with KV as a cache.

**Acceptance criteria:**

- AC-32.30: A shared pure resolver `resolveMemberVotes(member, deps)` SHALL return one `MemberBillPosition` per curated roll-call for the member, with `cast` normalized to `Aye`/`Nay`/`Present`/`Not Voting`, or `Did Not Serve` when the member is absent from that roll-call's roster. Both the widget (`useVotingRecord`) and the admin Bills matrix SHALL consume it, so they cannot diverge.
- AC-32.31: A House cast SHALL be matched by `bioguideId`; a Senate cast by `(lastName, state)` — consistent with the two KV roster cast shapes and the constraint that Senate.gov XML carries no bioguide.
- AC-32.32: `forAgainstUkraine` SHALL equal the sign of `computeValence(direction, action, directionMultiplier)`: `'for'` for `voted-pro`, `'against'` for `voted-anti`, `'n/a'` otherwise (Present, Not Voting, Did Not Serve, or `directionMultiplier === 0`).
- AC-32.33: A member absent from every curated roster SHALL resolve to all `Did Not Serve` / `inRoster:false` rows and SHALL NOT throw; a transient roster-fetch error for one roll-call SHALL degrade only that row to `Did Not Serve`, never failing the batch.
- AC-32.34: `resolveMemberVotes` SHALL resolve a member across **both** chambers — it gathers the curated votes for House and Senate and looks up each by the chamber-appropriate key (House by `bioguideId`, Senate by `(lastName, state)`) regardless of the member's current chamber. A member who never served in a chamber simply resolves to `Did Not Serve` for that chamber's roll-calls.
- AC-32.35: A chamber-switcher (e.g. `S001150`) SHALL surface their prior-chamber Ukraine votes and be scored on the combined record (clearing the FR-55 insufficient-record gate when the combined contributing-action count warrants).
- AC-32.36: Member casts SHALL be durably stored in a D1 `vote_casts` table, seeded by an idempotent CLI (`lw rosters seed`) that enumerates the curated roll-calls from the D1 `votes` table and fetches each roll-call's casts from upstream (Congress.gov house-vote members; Senate.gov XML). Re-running the seed against unchanged upstream SHALL not duplicate rows (UNIQUE on `(chamber, congress, session, roll_call, bioguide_id, last_name, state)`). Per-roll-call failures SHALL be written to `audit_log` (`action='roster_seed_error'`) and SHALL NOT abort the run; exit codes mirror `lw bills seed` (0 ok / 1 config error / 2 partial).
- AC-32.37: `GET /api/roll-call-rosters/{chamber}/{congress}/{session}/{rollCall}` SHALL serve the KV record when present and, on a KV miss, fall back to assembling the roster from D1 `vote_casts` (the existing `HouseRollCallRoster`/`SenateRollCallRoster` shape) and write that assembled record through to KV (self-healing). It returns 404 only when both KV and D1 are empty. KV is thus a cache and D1 the source of truth — a KV flush is re-filled from D1 on the next read OR the next `lw kv publish`. The response contract (`casts` Record for house, array for senate) SHALL be unchanged so no client change is required.
- AC-32.38: The seed CLI SHALL be wired into `scripts/cli.ts` as a `rosters` command group (`lw rosters seed`), reusing the `resolveRuntime` / `CongressClient` / `AuditLogger` / `D1Like` patterns established by `lw bills seed`.

**Full member durability (v4.1.2).** Member identity, the per-state roster, and the name-search index were the remaining KV-only, upstream-sourced data (no D1 backing — a KV wipe lost them). A single D1 `members` table SHALL become the durable source of truth for all three, with KV demoted to a projected cache.

- AC-32.39: A D1 `members` table SHALL durably store one row per current-Congress member with the fields needed to rebuild `member:v1:`, `state-members:v1:`, and `name-index:v1:` without any upstream fetch: `bioguide_id` (PK), `first`, `last`, `official_name`, `state` (two-letter), `chamber`, `district`, `party`, `photo_url`, `website`, `search_key`, `year_entered`, `is_non_voting`, `socials_json`, `sponsored_json`, `cosponsored_json`, `congress_update_date`, `last_freshness_check_at`. It SHALL be seeded by `lw members seed` — enumerating current members from Congress.gov, fetching per-member detail + sponsored/cosponsored legislation + socials (`unitedstates/congress-legislators`), freshness-gated on `congress_update_date` (`--force` bypasses), idempotent, per-member failures logged to `audit_log` (`action='member_seed_error'`), exit codes 0/1/2 mirroring `lw bills seed`.
- AC-32.40: `lw kv publish` SHALL project `member:v1:{bioguide}`, `state-members:v1:{state}`, `name-index:v1:{letter}` (+ `:meta`), and `roll-call-roster:v1:*` from D1 (`members` + `vote_casts`) with **no upstream fetch**. A KV flush followed by `lw kv publish` SHALL fully restore these records. The projected record shapes SHALL be byte-compatible with what the existing routes + widget consume (`MemberProfile`, the state-members `{stateCode,senators,house}` shape, the `NameIndexEntry` shards + meta, and the House-Record / Senate-array roster shapes).
- AC-32.41: `GET /api/members/{id}`, `/api/state-members/{state}`, `/api/name-search`, and `/api/roll-call-rosters/*` SHALL serve from KV when present and, on a KV miss, fall back to assembling the record from D1 and write-through caching it (self-healing) before returning 404/503. `api-members` SHALL keep stamping `partyPrior` at read time from `scores:v1:party-priors`.
- AC-32.42: `scores:v1:party-priors` is computed by `scripts/compute-party-priors.ts` from the curated bills + pre-fetched rosters in bundled `src/data/ukraineVotes.json`. **Deferred (documented exception, v4.1.2):** this remains the one curated-JSON-sourced KV record. It is not lost on a KV wipe (re-running the compute script regenerates it deterministically from committed bundled data), and it is render-time advisory (a Bayesian shrink target), so it is lower-risk than the member/cast data. Moving its source to D1 (`members.party` + `vote_casts`-resolved scores) is tracked as a follow-up; until then the bundled-JSON dependency is the known remaining non-D1 source. `cache:v1:*` (ephemeral HTTP cache) is intentionally not D1-backed.

- AC-32.44: **Publish reads from the typed D1 REST client and fails loudly.** The D1→KV publish (`lw kv publish`) SHALL read tables via the typed `D1Like` REST client (`scripts/lib/d1-client.ts`, the same one the seed CLIs use) — NOT by shelling out to `wrangler` and parsing stdout. Its read wrapper SHALL tolerate ONLY a "no such table" error (pre-migration env → empty set); any other read failure (network, auth/5xx, malformed JSON) SHALL propagate and abort the publish run, NOT be swallowed into an empty result. Rationale: the prior `execSync`-based reader was capped by execSync's 1 MiB stdout `maxBuffer` — a large `SELECT * FROM vote_casts` (~20k rows) overflowed it, the error was silently swallowed, and publish wrote `members=0 voteCasts=0` and reported success (v4.1.2 incident). The REST client removes the buffer ceiling, stdout-parsing, and shell-quoting entirely; fail-loud ensures a bad read can never ship a hollow cache.
- AC-32.43: **Selective member seeding.** `lw members seed` SHALL support targeting a subset so cleanup/retry does not re-touch the whole roster:
  - `--bioguide <id>` (repeatable): seed ONLY the given bioguide(s) — skips list enumeration entirely (zero list calls; one detail call per id). Invalid ids (not `^[A-Z][0-9]{6}$`) are rejected.
  - `--only-missing`: enumerate the current roster but seed ONLY bioguides absent from the D1 `members` table — existing rows incur ZERO upstream detail calls (the filter is a single `SELECT bioguide_id FROM members`, not per-member). This makes straggler cleanup after a partial run cheap and idempotent.
  - `--retry-failed`: seed ONLY bioguides with a recent `audit_log` row `action='member_seed_error'` (the failures from the prior run).
  When none are given, behavior is unchanged (full roster with the per-member freshness gate). The orchestrator's selection SHALL be expressed as an injected/optional filter so it is unit-testable without the network.
- AC-32.45: **Roster-key contract between producer and party-priors consumer.** `scripts/build-vote-rosters.ts` (producer) and `scripts/compute-party-priors.ts` (consumer) SHALL agree byte-for-byte on the roll-call key used to index `src/data/ukraineVotes.json` `rosters`. The canonical key SHALL be `` `${chamber}|${congress}|${session}|${rollCall}` `` with `chamber` the literal `'House'` or `'Senate'` (e.g. `House|117|2|65`). Rationale: AC-32.42 promises party-priors regenerate **deterministically** from committed bundled data; a silent key-format drift (the consumer reading `h/${c}/${s}/${rc}` while the producer writes `House|${c}|${s}|${rc}`) makes every `rosters[key]` lookup miss, so the script computes empty priors (`reps_seen=0`) while still exiting 0 — a hollow `scores:v1:party-priors` write that looks successful but degrades every rep to no-shrink. The key builder SHALL be a single shared/duplicated pure helper with the format asserted by a unit test that exercises both a known producer key and the consumer's lookup against a fixture roster, so the two cannot drift again silently. (Discovered v4.2.0 stg seed: `compute-party-priors` produced `reps_seen=0` against a freshly-built `ukraineVotes.json`.)
...
- AC-40.11: **Cache-key injectivity at every tier (NEW v4.1.0).** For any two structurally non-equal `CacheKey` values `a` and `b` (i.e., `!cacheKeyEquals(a, b)`), every tier's native key derivation SHALL produce a distinct storage address. Concretely:
  - The edge-tier URL adapter (`keyToUrl(key): URL`) supplied by the route handler SHALL incorporate both `key.kind` AND a canonical serialization of `key.params` into the URL. Encoding the kind alone (e.g., as a URL fragment) is insufficient when route identity lives in the request query string, because two requests with the same path but different params collapse to the same edge entry and cause cross-address response confusion.
  - The implementation lives in the helper `edgeKeyToUrl(target, upstreamPath, key)` in `proxy/routes/api-upstream.ts`; it appends `cacheKeyToDottedString(key)` to the URL pathname (with `#` separator) so the dotted form — same one the KV tier uses — distinguishes entries.
  - The KV-tier storage key already satisfies this via `KV_CACHE_PREFIX + cacheKeyToDottedString(key)`; this AC documents the invariant rather than introducing a new requirement for that tier.
  - The R2-tier path scheme SHALL likewise be injective on `(kind, params)`.
  - **Forbidden case (regression test required):** two GET requests to `/api/census/geocoder/geographies/onelineaddress` differing only in their `address` query parameter SHALL NOT share an edge-tier cache entry. The proxy SHALL issue two distinct upstream calls (modulo independent caching of identical addresses).
  - **Rationale.** The same defect surfaced twice in production: first on 2026-05-03 (initial incident) and again on 2026-05-25 (`trackukraine.com`, traced to the edge-tier `keyToUrl` adapter building its URL from the request pathname plus `#${k.kind}` only, dropping `key.params.qs`). Census-geocoder is the only canonical route that carries identity in the query string (all other cacheable routes encode identity in the pathname), so the defect went undetected each time until a real second address lookup was performed at a warm POP. See ADR-019.
...
- AC-43.7: The `lowConfidence` clamp on `scoreLabel` (FR-16) SHALL remain — at `'low'` tier the label reads "Limited record" variants ("Limited record — some support" / "Limited record — some opposition" / "Limited record"). At `'moderate'` the label reads as the full-tier would; the moderate desaturation is the only visual signal of partial confidence. This is intentional: the binary-clamp label change was itself part of Kaziem's "degrades mid-rangers" feedback — moving the nuance from copy to color is the design intent.
- AC-43.7a (REVISED v4.3.0): Score-band labels SHALL use "support"/"opposition" wording, never "supportive" or "leaning": `Strong support` (≥0.75), `Support` (≥0.35), `Some support` (≥0.1), `Mixed` (>−0.1), `Some opposition` (>−0.35), `Opposed` (>−0.75), `Strong opposition` (≤−0.75). The score-bar scale labels SHALL read `Opposed · Mixed · Support`. Rationale: the user flagged "supportive" + "lean" as confusing hedge words.
- AC-43.7b (REVISED v4.3.0): The "Based on N counted actions" justification line SHALL render in the default muted text color. The earlier red→amber→green hue gradient on this line (implemented as `confidenceColorFor`, never an approved spec AC) is REMOVED — it read as "low data = bad" and failed contrast in light mode. The confidence signal is carried solely by the AC-43.3 saturation of the score number plus the line's own tooltip.
...
- AC-52.24 (SUPERSEDED by FR-63 AC-63.7, v4.3.0): Originally the bill-direction context strip glossed what `direction_multiplier = +1 / −1` meant for the bill. Under FR-63 votes carry an explicit per-vote `direction` and there is no multiplier/inversion to explain. The strip now states the bill direction, notes it drives **sponsorship** scoring, and points researchers to set each vote's own direction in **Admin › Vote review**. The `ambiguous` case still renders just the bill direction with no per-vote gloss. Tests assert the new framing (no "inverted"/"multiplier" language).
...
- AC-21.6 (NEW v4.3.0): The **date** shown for a roll-call vote in the Ukraine Votes table SHALL be the **legislative calendar date** of the vote, i.e. `CuratedBillVote.actionDate` (a date-only `YYYY-MM-DD` string), NOT a calendar-day slice of `CuratedBillVote.date`. Rationale: Congress.gov's `recordedVotes[].date` is a **UTC timestamp**; evening roll calls in Washington (e.g. `2022-03-10T02:49:07Z` = the night of **March 9** EST) slice to the *following* UTC day, so the widget displayed dates one day later than the vote actually occurred. `actionDate` already carries the correct legislative date. When `actionDate` is absent/empty, the UI SHALL fall back to the `YYYY-MM-DD` slice of `date`. Tests SHALL cover: a late-night-UTC vote whose `actionDate` precedes its `date` slice renders `actionDate`; a vote with no `actionDate` renders the `date` slice.

### FR-59: Standalone Bill Seeding CLI + Full Roll-Call Ingest (NEW v4.1.0)

**Terminology note.** This is **seeding**, not backfilling. "Backfill" implies an existing dataset with gaps; the job described here populates from upstream truth (Congress.gov) and keeps it current. Every run is independent, environment-agnostic, and idempotent — the same code path produces byte-identical D1 state regardless of which env it targets or how many times it has run before.


**Problem.** As of UAT audit 2026-05-19, ingestion of bill metadata, cosponsors, votes, and `became_law` flags is structurally incomplete across the curated bill set:

- The 119th Congress (21 bills): **zero cosponsors, zero votes** — total miss.
- The 118th Congress (21 bills): 3/21 with cosponsors, 8/21 with votes — uneven, pattern consistent with a partial run.
- The 117th Congress (21 bills): 17/21 cosponsors, 11/21 votes — mostly populated, with a handful of stragglers.
- `became_law = 0` on all 63 bills, including known Public Laws (PL 117-109, PL 117-110, HR 2471 became law as appropriations, HR 815 became law as supplemental).
- Two bills mislabeled `direction='anti-ukraine'` (118-HR-2445 Special Inspector General, 118-S-2552 Ukraine Aid Oversight Act) — these are oversight bills with no aid-blocking provisions.

**Historical context — what broke.** The per-bill ingestion code (`proxy/services/import-bill.ts`'s `importBillFromCongress`) is structurally correct: it walks every action's `recordedVotes[]`, refreshes cosponsors via wipe-and-reload, and maps Congress.gov's `bill.laws[].length > 0` to `bills.became_law = 1`. The defect was the **driver layer**: the only path that called this orchestrator across all bills was `useAutoBackfill` (`src/admin/App.tsx:358`), a React effect running in the admin browser. It was whoami-gated, tracked progress in `localStorage`, processed 3 bills per chunk via `POST /api/admin/backfill-bills`, and **silently aborted on any chunk failure** (`console.error; return` without setting the completion flag). The audit's descending pattern (117th mostly done → 118th partial → 119th zero) matches the natural `bill_id ASC` ordering of a loop that stalled mid-flight. The job effectively required an admin to keep a browser tab open for the duration of every Congress.gov refresh, with no recovery if any chunk failed. Architecturally wrong: a multi-hour external-API ingest job has no business living in browser session state.

**Solution.** Move seeding out of the runtime stack entirely. The new home is a **standalone tsx CLI** at `scripts/bills/seed.ts` (invoked as `lw bills seed`), run by GitHub Actions on a cron schedule (and on-demand via `workflow_dispatch`). The CLI:

- Hits Congress.gov directly with its own API key.
- Writes to D1 via `wrangler d1 execute` or the D1 REST API (per-env binding).
- Iterates every row in `bills` ordered by `bill_id ASC`, calling the same orchestration logic as the deleted Worker route (extracted to a pure helper consumable from both runtimes during transition — see below).
- Writes structured `audit_log` rows for every operation so the runtime can observe ingest state.
- Exits when done. Has no Worker presence. Has no SPA presence. The legacy admin SPA `useAutoBackfill` hook and the `/api/admin/backfill-bills` route are **deleted**, not deprecated.

**Why a separate program, not a Worker scheduled handler:** the runtime Worker exists to serve reads and admin edits. Coupling a multi-hour external-API ingest job to it inherits the Worker CPU budget, the request-Worker rate limit on Congress.gov (shared with serving users), the Worker version-pinning on deploy, and a single failure domain. CI is the right home: it has minutes-long CPU budgets, it has its own secrets store, it can be re-run idempotently, and its failures don't degrade serving. This applies to every future ingest job (FR-60 full corpus, election data, FEC, etc.) — seeding is build/ops, not runtime.

**Note on the orchestrator.** `importBillFromCongress` previously lived in `proxy/services/import-bill.ts` pulling `D1` + `KV` bindings from `ProxyEnv`. To consume it from a CLI, the orchestration logic was extracted into `scripts/lib/bills/import-core.ts` (pure helper, no Worker types), with `proxy/services/import-bill.ts` becoming a thin adapter that wires Worker bindings into the core. The deleted SPA-driven route (`/api/admin/backfill-bills`) was the *only* runtime caller and is gone; `importBillFromCongress` stays exported from the adapter for any future per-bill admin actions.

**Note on scope.** Bringing the FULL Congress.gov bill corpus (~48k bills across 117/118/119) is scoped to FR-60. FR-59 is narrowly the *fix-what's-already-curated* milestone, scoped to the bills already present in `bills` (currently 63). FR-60 builds on the same CLI driver but adds: list-page discovery, raw-bytes R2 archive, classifier-flagged relevance, per-minisite scoping (FR-61/62).

**Acceptance Criteria:**

- AC-59.1: A standalone CLI subcommand SHALL exist at `scripts/bills/seed.ts` (wrapper) over `scripts/lib/bills/seed.ts` (pure core), invoked via `npx tsx scripts/cli.ts bills seed --env <dev|uat|stg|prod> [--limit N] [--after <bill_id>] [--force] [--concurrency N] [--congress N] [--local] [--dry-run]`. The program SHALL exit with code 0 on success (all bills processed or budget exhausted), code 1 on configuration error (missing API key, unreachable D1, bad `--env`), and code 2 on at-least-one-bill-failed (partial success — failures logged to audit_log; cron remains green by treating exit 2 as a warning, not a failure).
- AC-59.2: The CLI SHALL iterate every row in `bills` ordered by `bill_id ASC`, calling the extracted `importBillCore({ congress, type, number, force, actorEmail, traceId })` per row with deps `{ d1, congressClient, auditLog, kvInvalidate, log }`. The `force` flag defaults to `false` so the bill-level freshness gate (per AC-59.10) short-circuits unchanged bills to one API call each; `--force` on the CLI flips it to `true` for a full re-pull. The CLI SHALL emit one stdout line per bill processed in the format `[seed] {bill_id} {ok|cached|FAILED} {ms}ms { ...counters }` so the CI log is grep-able.
- AC-59.3: Per-bill failures SHALL NOT abort the loop. Each failure SHALL be written to `audit_log` with `action='bill_seed_error'`, `actor_email='ci@seed'`, `row_id=<bill_id>`, `reason=<upstream_error_message>`, `trace_id=<run_trace_id>`. The CLI continues to the next bill. The final exit summary SHALL include `{ processed, ok, failed, errors: [...] }`.
- AC-59.4: The CLI SHALL respect Congress.gov's 5,000 req/h rate ceiling by issuing at most 2,500 req/h sustained against the API (50% headroom — see ADR-020 placeholder for the rate-limit budget rationale). For the current 63-bill corpus this is a small concern; the limit becomes load-bearing under FR-60. The CLI SHALL honor `X-RateLimit-Remaining` / `Retry-After` headers and back off proactively.
- AC-59.5: After a clean run, the query `SELECT became_law, COUNT(*) AS n FROM bills GROUP BY became_law` SHALL include a result row `(became_law=1, n ≥ 4)` — i.e., at least 4 bills SHALL be flagged as having become law (HR-815, HR-2471, HR-6968, HR-7108 — the four known-PL bills in the current curated set). An integration test against `importBillCore` SHALL simulate Congress.gov returning `laws=[{number:'117-109'}]` and assert `bills.became_law=1` in the row after import.
- AC-59.6: The orchestrator's behavior of walking every action's `recordedVotes[]` (currently at `proxy/services/import-bill.ts:297–318`, moving to `scripts/lib/import-bill-core.ts`) SHALL be preserved as the source-of-truth for "every roll call referenced by every action of every bill." A regression test SHALL feed a synthetic actions feed containing 5 recordedVotes across 3 actions and assert 5 vote rows are written.
- AC-59.7: Two specific mislabels SHALL be corrected:
  - `118-HR-2445` direction `'anti-ukraine'` → `'neutral'`. Rationale: the Special Inspector General for Ukraine Assistance Act establishes oversight; it does not block aid. Same posture as 118-HR-855 per `curated-bills-v0.1.0.md` v0.1.1.
  - `118-S-2552` direction `'anti-ukraine'` → `'neutral'`. Rationale: the Ukraine Aid Oversight Act mirrors the same SIG-style oversight framing.
  These corrections SHALL be expressed as a one-shot SQL migration `migrations/d1/0010_v4_1_0_direction_corrections.up.sql` with corresponding `_rollbacks` file, applied as part of the v4.1.0 deploy. The `audit_log` SHALL record both updates with `actor_email='v4.1.0@release'` and `reason='AC-59.7'`.
- AC-59.8: A researcher-visible **Data freshness panel** in the admin SPA (under Settings ▸ Data freshness, registered alongside Poll Status) SHALL display **research-relevant state** about the bill corpus, not operator-relevant CI state. The panel SHALL show: total bill count, count per congress, count per direction, count of bills with `became_law=1` (as "Became law: N of M" so researchers can confirm Public-Law bills are populated post-ingest), the distribution of `bills.last_freshness_check_at` ("N bills last refreshed within 24h / 7d / 30d / older"), and a "stale bills" list (bills with `last_freshness_check_at` older than 24h, capped at 20). The panel SHALL NOT show operator concepts ("cron tick N", "job stalled at chunk M", "next run at HH:MM"). The data shape comes from `GET /api/admin/data-freshness` — primary source-of-truth is the `bills` table aggregations; the most recent `audit_log` row with `action IN ('import_bill','bill_seed_error')` is surfaced as "last refresh attempted: <timestamp>" context only. Long-term home of this panel is open and tracked under the post-v4.1.0 minisite-platform planning.
- AC-59.9: The browser-driven seeding SHALL be **removed**, not deprecated. Files to remove:
  - The `useAutoBackfill` hook (`src/admin/App.tsx`), the `BACKFILL_VERSION`/`BACKFILL_DONE_FLAG`/`BACKFILL_CURSOR_KEY` constants, and the `useEffect` call that invokes it.
  - The `handleBackfillBills` function in `proxy/routes/api-admin.ts` and the route dispatch line `if (resource === 'backfill-bills') return await handleBackfillBills(...)`.
  - Any tests under `tests/` that exercise these paths (the test files themselves are deleted in the same commit).
  Removal is in-scope for v4.1.0 — not deferred. The new CLI is the sole driver from v4.1.0 onward.
- AC-59.10: **Idempotency + minimum-fetch posture.** The seed CLI is byte-identically idempotent: two consecutive runs against unchanged upstream state SHALL produce identical D1 state.
  - **Bill-level gate:** every run starts with one call to `/v3/bill/{c}/{type}/{n}` per bill. If the response's `bill.updateDate` matches the stored `bills.congress_update_date`, the CLI SHALL short-circuit — no summaries, actions, cosponsors, or per-vote calls are issued. The whole bill costs exactly 1 API call.
  - **Vote-level gate:** when the bill-level gate misses (bill changed), per-roll-call `votes.congress_update_date` SHALL short-circuit per-vote updates. Votes that haven't moved are skipped.
  - **Default `force=false`:** the CLI runs without `--force` by default. The freshness gates are the cost-control mechanism; `--force` exists only for "I just edited the curated seed and want to re-pull everything."
  - **Byte-identical assertion (test-enforced):** an integration test SHALL run the seed CLI twice in succession against an unchanged fake Congress.gov + a populated D1 state, then assert: (a) `processed` equals the bill count both times; (b) zero new rows are written to `bills`, `votes`, `bill_cosponsors`, `bill_actions` on the second run (row-count diff = 0); (c) zero `audit_log` entries with `action='import_bill'` are written on the second run (each bill should short-circuit cleanly without writing the per-import audit row).
  - **Expected steady-state cost:** for the current 63-bill corpus, a no-changes run SHALL produce ≤ 63 API calls (~45s at 5,000/h ceiling). A cold run with no prior data SHALL produce ≤ 500 API calls.
  - **Worker rate-limit posture:** the orchestrator's `congress_update_date` check is the same one the Worker adapter uses for one-bill admin actions, so the no-change quote-saving discipline applies identically to per-bill researcher imports (no behavior change there).
- AC-59.11: A GitHub Actions workflow `.github/workflows/seed-bills.yml` SHALL drive the CLI: cron schedule (default: every 6h on dev/uat, every 12h on stg/prod), `workflow_dispatch` for manual runs, per-env matrix, secrets-store wiring (`CONGRESS_API_KEY`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`). Workflow exits green on CLI exit 0 *or* 2 (partial success — failures are in audit_log, not in CI alerts); exits red on CLI exit 1 (config error).
- AC-59.12: **Environment-agnostic execution.** The orchestration code path (`importBillCore`, `seedBills`) SHALL NOT branch on env name. `resolveRuntime` picks D1 / KV bindings + Congress API key from env-named config; downstream code only consumes the resolved interfaces (`D1Like`, `CongressClient`, `AuditLogger`, `kvInvalidate`). Default D1 transport for every env is `--remote`; the CLI accepts `--local` for fast dev iteration via the local wrangler binding. The same `npx tsx scripts/cli.ts bills seed --env <X>` invocation SHALL produce identical behavior across `X ∈ {dev, uat, stg, prod}` modulo the bound D1/KV namespace.

### FR-59 (v4.1.1 addendum): Keyword-relatedness classification of ingested posts

**Problem.** The social poll already matches each ingested post body against the active `social_keyword_watches` (`src/ingest/poll-worker.ts` → `matchKeywords`) and stores the hits in `social_post_queue.matched_keywords`, but every row is enqueued with `status='pending'` regardless of whether anything matched. So a researcher's curation feed is polluted with off-topic posts (a member's unrelated tweets), and there is no stored, queryable distinction between "relevant to our watches" and "noise."

**Solution.** Classify at ingest time and store it. A post that matched ≥1 active keyword watch is `pending` (enters the curation feed); a post that matched none is `unrelated` (stored and persistent — never re-fetched — but outside the default feed). Posts are still stored discretely (dedup on `(platform, platform_post_id)`); classification is just the stored `status`. Manual/human adds are never auto-classified.

**Acceptance Criteria:**

- AC-59.20: When the poll loop enqueues a post whose body matched ≥1 active keyword watch, the stored `social_post_queue.status` SHALL be `'pending'`.
- AC-59.21: When the poll loop enqueues a post that matched **no** keyword watch, the stored status SHALL be `'unrelated'` — retrievable via `listQueue({ status: 'unrelated' })` but excluded from the default (`related`) feed view.
- AC-59.22: A post added via `POST /api/admin/ingest/queue` (manual/direct add) SHALL be stored `'pending'` regardless of keyword matches — explicit human intent overrides auto-classification. `enqueuePost` SHALL accept an optional `status` override; the poll path omits it (deriving from `matchedKeywords`), the manual path passes `'pending'`.
- AC-59.23: Migration `0011` SHALL reclassify pre-existing rows that are `status='pending'` AND `matched_keywords IS NULL` AND `reviewed_at IS NULL` to `'unrelated'`, leaving any reviewed row (`curated`/`dismissed`, or any non-null `reviewed_at`) untouched. The migration SHALL be idempotent; its rollback reverses the same predicate. No schema/CHECK change (status has been free TEXT since `0005`; `idx_social_post_queue_status` already exists). The queue is admin-only curation state — this classification SHALL NOT affect the public KV publish path.

### FR-60: Direct-to-Member Embed Deep Link (NEW v4.1.1)

**Problem.** The admin SPA's PersonProfileView (`src/admin/components/PeopleTab.tsx`) renders a "Widget Preview" panel so a researcher can see how the public widget presents the member they are curating. Today that panel is a bare `/embed` iframe with the instruction *"Search for {name} in the widget below to see their current profile."* The researcher must manually re-type a name they are already looking at, the embed lands on the generic address/name-search entry screen rather than the member, and the panel's static copy still reads "Widget Preview — trackukraine.com" regardless of which env (and therefore which dataset) the researcher is working against. The preview does not show the profile of the person whose profile it is.

**Solution.** Add a **direct-to-member deep link** to the embed so the iframe can open straight onto a single member's `RepDetail`, with no search interaction. The embed page accepts a `?bioguide=<id>` query parameter and forwards it to the `<voter-info-widget>` element as a `bioguide` attribute. When the widget boots with a `bioguide` attribute, it fetches the existing `/api/members/{bioguideId}` route (FR-32 AC-32.1 / AC-24.2 — already returns `bioguideId`, `officialName`, `first`, `last`, `state`, `district`, `chamber`, `party`, `photoUrl`, `website`, `yearEntered`: the complete set needed to construct a seed `Representative`), constructs the `Representative`, and renders the same `RepDetail` the address and name-search flows render — bypassing `AddressInput` and `NameSearchInput` entirely. The admin SPA builds the iframe `src` as `${origin}/embed?bioguide=${bioguideId}` against the **same origin** it is served from (so the preview reflects the env the researcher is editing). No backend route change is required; `/api/members/{id}` already returns the full shape. This is purely a widget-side deep-link entry point plus a one-line embed-HTML pass-through plus an admin-SPA iframe-`src` change.

This deep link is **additive** and harness-only in spirit: it does not change the public embed snippet that integrators paste (they never set `bioguide`), and the default no-`bioguide` boot is byte-for-byte the existing address/name-search entry screen (FR-31 unaffected).

**User story (US-10)**: *As a researcher viewing a member's profile in the admin SPA, I want the widget preview to open directly on that member's public profile — without me searching for them — so I can verify how my curation renders for the public in one glance.*

**Acceptance criteria:**

- AC-60.1: `buildEmbedHtml` (`proxy/routes/preview.ts`) SHALL read a `bioguide` query parameter from the request URL and, when present and matching the bioguide shape `^[A-Z][0-9]{6}$`, set it as a `bioguide` attribute on the `<voter-info-widget>` element it mounts. When the parameter is absent or fails the shape check, the element SHALL be mounted with no `bioguide` attribute (current behavior, unchanged). The parameter value SHALL be validated against the shape before being written into the HTML (no raw interpolation of the query value into markup) to preserve the embed's existing CSP/injection posture. To accept the parameter, `buildEmbedHtml` SHALL take the request URL (or the parsed `bioguide` value) as an argument; the router (`proxy/router.ts`) SHALL pass it through from the matched request.
- AC-60.2: `<voter-info-widget>` (`src/embed.tsx`) SHALL add `bioguide` to its `observedAttributes` and pass the attribute value through to `VoterInfoWidget` as an `initialBioguide` prop (omitted/empty when the attribute is absent). Changing the `bioguide` attribute after mount SHALL re-render with the new member (consistent with the existing `attributeChangedCallback` re-render contract).
- AC-60.3: When `VoterInfoWidget` receives a non-empty, shape-valid `initialBioguide`, on first render it SHALL fetch `${apiBase}/api/members/{bioguideId}`, construct a `Representative` from the returned record (`name` = `"{last}, {first}"` to satisfy the Senate roster-matching contract used by `useVotingRecord`; `party` mapped `D→Democratic`, `R→Republican`, `I→Independent`, else passthrough; `partyAbbreviation`, `state` **normalized to the two-letter code** (the `/api/members/{id}` route returns the full Congress.gov state name, e.g. `"Illinois"`, but Senate roster matching in `useVotingRecord` compares against the two-letter code on each cast, e.g. `"IL"` — without normalization every Senator's votes silently fail to match and the profile shows "No Ukraine-related votes"; a value already two letters passes through), `district`, `chamber` = `chamber.toLowerCase()`, `photoUrl` via `sanitizeUrl`, `officialWebsiteUrl` via `sanitizeUrl`, `yearEntered`), and render that member's `RepDetail`. It SHALL NOT render `NameSearchResultsPanel` or perform a `/api/name-search` request for the deep-link path. The member's `RepDetail` SHALL be open on load. (Entry-control visibility in this mode is governed by AC-60.8.)
- AC-60.4: While the `/api/members/{bioguideId}` deep-link fetch is in flight, the widget SHALL show a loading affordance (the existing loading idiom is acceptable) in place of the detail. The deep-link fetch SHALL be abortable/ignored if the `initialBioguide` changes before it resolves, so a stale response cannot overwrite a newer selection.
- AC-60.5: If the `/api/members/{bioguideId}` deep-link fetch returns 404 (`member_not_found`) or any non-OK status, or fails the in-flight, the widget SHALL degrade to the normal entry screen (address + name-search, no detail open) without throwing — an unknown bioguide produces the ordinary widget, never a crashed mount. Rationale: PersonProfileView keys on a `bioguideId` that may belong to a person without a current-Congress member record (a tracked non-member); the preview must not error for them.
- AC-60.6: The admin SPA PersonProfileView (`src/admin/components/PeopleTab.tsx`) SHALL set the preview iframe `src` to `${window.location.origin}/embed?bioguide=${encodeURIComponent(bioguideId)}` (same origin as the SPA). The panel's static heading SHALL NOT hard-code a domain that does not match the served origin; it SHALL read "Widget Preview" (no domain) or include the env/origin derived at runtime. The instruction copy that tells the researcher to manually search for the person SHALL be removed for the deep-link case (the member loads automatically); a brief caption MAY note the preview reflects the current environment's data.
- AC-60.7: The deep link SHALL NOT alter the public embed contract: a `/embed` request with no `bioguide` parameter SHALL produce HTML identical in behavior to the pre-FR-60 embed (a bare `<voter-info-widget>` with only `api-base` set), and the integrator-facing snippet documented elsewhere SHALL NOT gain a required `bioguide` attribute.
- AC-60.9: **Embed inline scripts carry a per-response CSP nonce.** The `/embed` HTML contains two trusted inline `<script>` blocks (the widget mount and the iframe auto-resize notifier). The route's `Content-Security-Policy` sets `script-src 'self' …` with no `'unsafe-inline'`, which blocks those inline scripts — the widget never mounts. The Worker SHALL generate a fresh, unguessable nonce per `/embed` response (e.g. `crypto.randomUUID()`), emit it as the `nonce` attribute on every inline `<script>` `buildEmbedHtml` produces, and include `'nonce-<value>'` in the response's `script-src` directive. `'unsafe-inline'` SHALL NOT be added. `buildEmbedHtml` SHALL accept the nonce as an argument so the same value appears in both the markup and the header. Rationale: this is a latent pre-FR-60 defect (the embed's CSP contradicted its own inline scripts); it is fixed here because the deep link cannot render without the embed mounting. A nonce preserves the strict no-`unsafe-inline` posture appropriate for a publicly iframe-embedded surface.
- AC-60.8: **Single-member view hides the entry controls.** When the deep-link path is active (a shape-valid `initialBioguide` is loading or has resolved to a member, and the user has not started their own address lookup or name search), the widget SHALL NOT render the `AddressInput` or `NameSearchInput` entry controls — the embed presents only that member's profile. When the deep-link fetch fails the shape check, 404s, or errors (AC-60.5), the widget falls back to the ordinary entry screen and the controls SHALL render normally. Rationale: the admin profile preview (AC-60.6) is a single-subject surface — showing a "look up your reps" form above a pre-loaded member is noise; the researcher wants to see only the person whose profile they are curating.

- AC-60.10: **Profile layout — preview beside the work, not below it.** In the admin `PersonProfileView`, on wide viewports (≥ 1100px) the view SHALL render as two columns. The **left** column (~40% width) holds ALL of the bio/work content top-to-bottom: the back link, the profile header banner, the stat cards, Social monitoring, Live Feed Search, and Quotes. The left column SHALL be a flex column in which **Quotes is the last section and flex-grows to fill the remaining height** the (taller) preview creates — so the column has no distributed gaps — with the quote list scrolling internally (`overflow-y: auto`) rather than stretching the column. The **right** column (~60% width, min ~420px) holds only the Widget Preview, which SHALL be `position: sticky` so it stays in view while the researcher scrolls/edits the left column, and SHALL span the full height of the left column from the top. The Recent Ingested Posts list SHALL span the full width below both columns. Below 1100px (tablet portrait and smaller) the layout SHALL collapse to a single column ordered: back → profile header → stat cards → **Widget Preview** → Social monitoring → Live Feed → Quotes → Recent Ingested Posts (preview kept near the work, not "miles away"); in single-column mode Quotes does not flex-grow or scroll internally. The Social monitoring panel SHALL be collapsible and collapsed by default; its collapsed header SHALL still surface the freshness/issue summary (the `FreshnessBadge`: oldest-update + failing/stale/never-polled counts) so monitoring problems remain obvious without expanding it.

- AC-60.14: **Tabbed left column.** The admin `PersonProfileView` left column SHALL present a tablist (`role="tablist"`) of `Quotes` | `Social Feed` | `Bills`; `Quotes` is selected on load and on member switch. The back link, profile header, stat cards, and the collapsible Social monitoring panel SHALL render **above** the tab bar (always-visible context) regardless of the active tab. The Bills tab SHALL NOT render for people without a Congress identity (no `bioguideId`/MoC record).
- AC-60.15: Selecting a tab shows only that body — `Quotes`→the quotes list; `Social Feed`→the stored ingested posts (AC-60.16) plus the persisting re-pull controls; `Bills`→the vote matrix (AC-60.21). Switching tabs SHALL NOT reload the widget-preview iframe.
- AC-60.16: **Related/unrelated feed filter.** The Social Feed defaults to showing only **related** posts (`matched_keywords` non-empty / `status='pending'`+`curated`); a **"Show unrelated" checkbox** (off by default) additionally reveals `unrelated` posts. The ephemeral "Live Feed Search" UI is removed; re-pulling uses the persisting `poll-handle` path (AC-60.22).
- AC-60.17: **Resizable preview.** On ≥1100px a draggable separator (`role="separator"`, `aria-orientation="vertical"`) sits between the left column and the preview; dragging resizes the preview pane within 30%–75% of the width; ArrowLeft/ArrowRight nudge ±2%; the chosen percentage persists in `localStorage["tk-admin-profile-layout"].previewPct`.
- AC-60.18: **Collapsible preview.** The preview can be collapsed; collapsed, the left column fills the full width and a slim vertical re-open strip shows on the right edge; the collapsed state persists in `localStorage["tk-admin-profile-layout"].previewCollapsed`.
- AC-60.19: **Full-height panes.** On ≥1100px the profile grid is the scroll boundary (`height: calc(100vh - offset)`, `overflow:hidden`); the left column and the preview scroll independently; the preview iframe fills the pane height (`height:100%`), so the embedded widget reflows responsively to the pane width.
- AC-60.20: Below 1100px the layout is a single column ordered back → header → stat cards → Widget Preview → tab bar → tab body; the divider/resize/collapse controls are not rendered.
- AC-60.21: **Bills matrix.** The Bills tab renders a summary StatCard row (bills tracked / for Ukraine / against) plus a matrix table: Bill (number+short title) | Date | Position (Aye/Nay/Present/Did Not Serve, valence-colored) | Outcome (became law?) | For/Against Ukraine, sourced from `resolveMemberVotes` (live roster fetches). Distinct loading, empty ("No Ukraine-related votes recorded."), error, and member-not-found states.
- AC-60.22: **Re-poll refreshes the posts feed.** When a re-poll (single handle `Re-poll`, or `Refresh all`) reports completion, the admin person-profile SHALL re-fetch `GET /api/admin/ingest/queue?bioguideId={id}` so newly-ingested posts appear in the feed without a manual page reload. (Defect: the queue `useEffect` previously omitted the reload trigger from its dependency array, so a re-poll refreshed handle rows but not the posts list.)

### FR-61: Researcher vs Admin UI split (NEW v4.3.0)

**Problem.** The admin SPA presents one flat navigation to every signed-in user. Non-technical researchers (the primary users) see operator-only surfaces — Cache, Sync status, App config, and the forthcoming API-quota view — which are confusing and irrelevant to the curation workflow. We want researchers to see only the research workflow, and operator/config surfaces to appear only for a small shortlist of admins.

**Posture (IMPORTANT).** This split is a **cosmetic UI affordance, not a security boundary.** Authorization stays at the edge: Cloudflare Access gates `/admin*` and `/api/admin/*`, and every CF-Access-verified user remains fully trusted and fully authorized at the API layer (memory `feedback_admin_auth_at_edge`, ADR-020). The split only decides **what the SPA renders**, so non-technical researchers aren't shown operator tooling. A determined researcher can still reach any `/api/admin/*` route directly; that is acceptable because all CF-Access users are trusted. See ADR-020 for why we accept an app-level email hint here despite the "no allowlist code" guidance.

**Acceptance criteria:**
- AC-61.1: `GET /api/admin/config` SHALL return a boolean `isAdmin` field. It is `true` when the verified actor email (already extracted from the CF Access JWT, FR-50) is a case-insensitive member of the `ADMIN_EMAILS` environment list, OR when `ADMIN_EMAILS` is empty/unset (fail-open: with no list configured, everyone is an admin, preserving today's behavior). The Worker SHALL NOT use `ADMIN_EMAILS` to authorize or reject any request — it only reports the hint.
- AC-61.2: `ADMIN_EMAILS` SHALL be a comma-separated, case-insensitive list of emails set per-env in `wrangler.toml` `[vars]`. Whitespace around entries SHALL be trimmed. An unset or empty value yields `isAdmin = true` for everyone (AC-61.1 fail-open).
- AC-61.3: The admin SPA megamenu SHALL group navigation into **researcher** sections (Workspace, Curation, Help) always visible, and an **Admin** section (Config sub-surfaces: Keywords, Tags, Sync status, Data freshness, Cache, App config, API quota) rendered ONLY when `isAdmin` is true. While `isAdmin` is still loading, the Admin section SHALL be hidden (it appears once config resolves).
- AC-61.4: Operator/config surfaces SHALL live under a single **Config** area (Settings routes) reachable from the Admin menu group. The researcher-facing Curation and Workspace groups SHALL NOT embed config knobs inline (consistent with CLAUDE.md "Settings is the home for cross-cutting knobs").
