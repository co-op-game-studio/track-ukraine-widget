...
- AC-40.11: **Cache-key injectivity at every tier (NEW v2.7.1, bugfix 2026-05-25).** For any two structurally non-equal `CacheKey` values `a` and `b` (i.e., `!cacheKeyEquals(a, b)`), every tier's native key derivation SHALL produce a distinct storage address. Concretely:
  ...
  - **Rationale.** On 2026-05-25 a production incident on `trackukraine.com` was traced to the edge-tier `keyToUrl` adapter building its URL from the request pathname plus `#${k.kind}` only, dropping `key.params.qs`. Census-geocoder is the only canonical route that carries identity in the query string (all other cacheable routes encode identity in the pathname), so the defect went undetected until a real second address lookup was performed at a warm POP. See ADR-015.
