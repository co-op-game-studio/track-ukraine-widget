...
describe('api-admin: POST /backfill-bills (chunked re-import)', () => {
  function seedBills(d1: FakeD1, ids: string[]): void {
  ...
  it('GET on /backfill-bills returns 400 method_not_allowed', async () => {
    const env = makeEnv(new FakeD1(), new FakeKV());
    const r = await call(env, 'GET', 'backfill-bills');
    expect(r.status).toBe(400);
  });

  it('processes a chunk and returns next_after when more remain', async () => {
    (importBillFromCongress as unknown as { mockClear: () => void }).mockClear();
    const d1 = new FakeD1();
    seedBills(d1, ['117-HR-1', '117-HR-2', '117-HR-3', '117-HR-4', '117-HR-5']);
    const env = makeEnv(d1, new FakeKV());
    const r = await call(env, 'POST', 'backfill-bills?limit=2');
    expect(r.status).toBe(200);
    const body = r.json as {
      processed: number; ok: number; failed: number;
      next_after: string | null; done: boolean;
    };
    expect(body.processed).toBe(2);
    expect(body.ok).toBe(2);
    expect(body.failed).toBe(0);
    expect(body.next_after).toBe('117-HR-2');
    expect(body.done).toBe(false);
  });

  it('returns done=true and next_after=null when fewer rows than limit', async () => {
    const d1 = new FakeD1();
    seedBills(d1, ['117-HR-1']);
    const env = makeEnv(d1, new FakeKV());
    const r = await call(env, 'POST', 'backfill-bills?limit=10');
    expect(r.status).toBe(200);
    expect(r.json).toMatchObject({ done: true, next_after: null });
  });

  it('cursors past ?after=…', async () => {
    const d1 = new FakeD1();
    seedBills(d1, ['117-HR-1', '117-HR-2', '117-HR-3']);
    const env = makeEnv(d1, new FakeKV());
    const r = await call(env, 'POST', 'backfill-bills?after=117-HR-1&limit=10');
    expect(r.status).toBe(200);
    const body = r.json as { processed: number; summary: Array<{ bill_id: string }> };
    expect(body.processed).toBe(2);
    expect(body.summary.map((s) => s.bill_id)).toEqual(['117-HR-2', '117-HR-3']);
  });

  it('records per-bill errors in the summary without aborting the chunk', async () => {
    const importer = importBillFromCongress as unknown as {
      mockImplementationOnce: (fn: (...a: unknown[]) => unknown) => void;
    };
    importer.mockImplementationOnce(() => { throw new Error('boom'); });
    const d1 = new FakeD1();
    seedBills(d1, ['117-HR-1', '117-HR-2']);
    const env = makeEnv(d1, new FakeKV());
    const r = await call(env, 'POST', 'backfill-bills?limit=10');
    expect(r.status).toBe(200);
    const body = r.json as {
    ...
    const failed = body.summary.find((s) => !s.ok);
